...
5. `dpr` вычисляется **после** сортировки: первый (наименьший) размерный сегмент задаёт `base_width`/`base_height`, поэтому все коэффициенты ≥ 1.
...
| `format` | `str` \| `None` | итоговый формат; `"auto"`/`""` → формат исходника (если исходник не картинка — `jpg`); не задан → настройки `format` → формат исходника |
