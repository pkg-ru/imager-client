...
| `sort` | `boolean` | `false` | sort size segments in `GetAssets` by `width`/`height` (see below) |
...
5. `dpr` is computed **after** sorting: the first (smallest) size segment defines `baseWidth`/`baseHeight`, so all ratios are ≥ 1.
